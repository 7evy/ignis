...
class HandoverRandomizationStep(RandomizationStep):
    ...
    @staticmethod
    def randomize_people(gd, user_config, characters, classes, rand, people):
        dirty = False
        for rid in people:
    ...
    @staticmethod
    def randomize_person(gd, user_config, rand, characters, classes, rid, replacement):
        ...
        if user_config.randomize_classes:
            fe14_utils.apply_randomized_class_set(
                gd, characters, classes, gd.string(rid, "aid"), rid, replacement, rand
            )
        ...
        fe14_utils.apply_randomized_stats(
            gd, rand, rid, rid, strategy, user_config.passes
        )
